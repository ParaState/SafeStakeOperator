...
pub fn test1() {
    ...
    println!(
        "Keystore decrypted with bad password. {:.2?}",
        now.elapsed()
    );
    ...
    println!(
        "Keystore decrypted with good password. {:.2?}",
        now.elapsed()
    );
}
...
pub fn test2() -> Result<(), Error> {
    ...
    scrypt(
        GOOD_PASSWORD,
        &salt,
        &ScryptParams::new(log2_int(262144) as u8, 8, 1).map_err(Error::ScryptInvalidParams)?,
        &mut dk,
    )
...
pub fn test3() -> Result<(), Error> {
    ...
    pbkdf2::<Hmac<Sha256>>(GOOD_PASSWORD, &salt, 262_144, &mut dk);
    println!("pbkdf2 kdf, 262_144 rounds. {:.2?}", now.elapsed());

    let now = Instant::now();
    pbkdf2::<Hmac<Sha256>>(GOOD_PASSWORD, &salt, 4096, &mut dk);

...
use crate::network::io_committee::{IOChannel, IOCommittee};
...
use types::{
    ChainSpec, Domain, Epoch, EthSpec, Fork, SignedRoot, SignedVoluntaryExit, VoluntaryExit,
};
...
async fn get_beacon_state_fork(client: &BeaconNodeHttpClient) -> Result<Fork, DvfError> {
    Ok(client
        ...
        .await
        .map_err(|e| {
            DvfError::BeaconNodeStateForkError(format!("Failed to get get fork: {:?}", e))
        })?
        .ok_or(DvfError::BeaconNodeStateForkError(
...
pub async fn get_distributed_voluntary_exit<T: IOCommittee<U>, U: IOChannel, E: EthSpec>(
    ...
    let epoch = get_current_epoch::<E>(genesis_data.genesis_time, &spec)
        .ok_or(DvfError::BeaconNodeGenesisError)?;
    let validator_index =
        get_validator_index_for_exit(&client, validator_pubkey, epoch, &spec).await?;
...
pub async fn test_distrubuted_voluntary_exit() {
    ...
    let futs = (0..IDS.len()).map(|i| async move {
        let io = Arc::new(
            SecureNetIOCommittee::new(IDS[i], ports_ref[i], IDS.as_slice(), addrs_ref.as_slice())
                .await,
        );

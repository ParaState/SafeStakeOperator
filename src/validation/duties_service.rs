...
use crate::validation::{
    ...
    http_metrics::metrics::{self, get_int_gauge, set_int_gauge, ATTESTATION_DUTY},
...
async fn poll_beacon_attesters<T: SlotClock + 'static, E: EthSpec>(
    ...
    update_per_validator_duty_metrics::<T, E>(duties_service, current_epoch, current_slot).await;
    ...
    update_per_validator_duty_metrics::<T, E>(duties_service, next_epoch, current_slot).await;
...
async fn get_uninitialized_validators<T: SlotClock + 'static, E: EthSpec>(
    duties_service: &Arc<DutiesService<T, E>>,
    epoch: &Epoch,
    local_pubkeys: &HashSet<PublicKeyBytes>,
) -> Vec<u64> {
    let attesters = duties_service.attesters.read();
    let pubkeys = local_pubkeys
        .iter()
        .filter(|pubkey| {
            attesters
                .get(pubkey)

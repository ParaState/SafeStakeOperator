...
use validator_dir::{BuilderError, ValidatorDir};
...
impl ShareBuilder {
    ...
    fn build_insecure_distributed_voting_keypair(
        ...
        let keystore = KeystoreBuilder::new(&operator_key_pair, INSECURE_PASSWORD, "".into())
            .map_err(|e| {
                BuilderError::InsecureKeysError(format!(
                    "Unable to create keystore builder: {:?}",
                    e
                ))
            })?
            .kdf(insecure_kdf())
            .build()
            .map_err(|e| {
                BuilderError::InsecureKeysError(format!("Unable to build keystore: {:?}", e))
            })?;
...
pub fn build_deterministic_distributed_validator_dirs(
    ...
    for i in 0..validator_ids.len() {
        let key_pack =
            generate_deterministic_threshold_keypairs(validator_ids[i], &node_ids, threshold);

...
use crate::node::contract::{
    DEFAULT_TRANSPORT_URL, NETWORK_CONTRACT, REGISTRY_CONTRACT, SELF_OPERATOR_ID, CONFIG_CONTRACT
};
...
impl Config {
    ...
    pub fn from_cli(cli_args: &ArgMatches, log: &Logger) -> Result<Config, String> {
        ...
        let config_contract: String = parse_required(cli_args, "config-contract")?;
        info!(log, "read config contract"; "config-contract" => &config_contract);
        CONFIG_CONTRACT.set(config_contract).unwrap();
